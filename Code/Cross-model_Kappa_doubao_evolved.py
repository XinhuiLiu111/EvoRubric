# scoring_with_best_template.py using Doubao
# 使用最佳评分标准对测试集进行评分并计算性能指标 - 豆包版本
...
class BestTemplateScorer:
    ...
    def _load_template(self, template_path: str):
        ...
        # 获取评分标准内容
        scoring_criteria = template_row['最佳评分标准(glm)'].values[0]
        ...
        base_instruction = '''You are an experienced secondary school English teacher. Please evaluate the following essay following this systematic process:
                ...
                - derive score through independent critical evaluation
                '''
    ...
    def evaluate(self, essays: List[str], human_scores: List[int]) -> Dict[str, Any]:
        ...
        # 定义一个包装函数来跟踪活跃线程数
        def score_with_counter(essay, idx):
            ...
            try:
                # print(f"【线程状态】当前活跃评分线程数: {current_active}/{self.max_workers}")
                # 调用实际的评分函数
                result = self._score_single_essay(essay)
                return result, idx
            finally:
                # 减少活跃线程计数
                with active_threads_lock:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for idx, essay in enumerate(essays):
                future = executor.submit(score_with_counter, essay, idx)
            ...
            with tqdm(total=len(essays), desc="评分进度") as pbar:
                for future in concurrent.futures.as_completed(futures):
                    try:
                        (score, evaluation), idx = future.result()
                        scores[idx] = score
                    ...
                    pbar.update(1)
                    # 在进度条上显示当前活跃线程数
                    pbar.set_postfix(active_threads=f"{active_threads}/{self.max_workers}")
...
def main():
    """主函数"""
    # 命令行参数
    parser = argparse.ArgumentParser(description="使用最佳评分标准对测试集进行评分 - 豆包版本")
    parser.add_argument('--dataset', default='ASAP', help="数据集名称")
    parser.add_argument('--essay_set', default=1, type=int, help="作文集编号")
    parser.add_argument('--model', default='doubao', help="使用的模型，默认为glm")
    parser.add_argument('--temperature', default=0.3, type=float, help="模型温度参数")
    parser.add_argument('--output_dir', default='cross-model-doubao(glm-rubric)', help="输出目录")
    ...
    # 输出结果
    # print("\n评分结果:")
    print(f"\n测试集大小: {len(test_essays)}篇作文")
    print(f"评分结果:加权Kappa系数: {results['kappa']:.4f}")
